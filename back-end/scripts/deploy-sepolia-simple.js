...
async function main() {
  console.log("🚀 开始部署到 Sepolia 测试网 (简化版)...");
  ...
  try {
    ...
    // 9. 为 MockUniswapIntegration 预存代币
    ...
    // 设置简单的交换比率（1:1）
    await mockUniswapIntegration.setExchangeRate(usdcAddress, wethAddress, 10000);
    await mockUniswapIntegration.setExchangeRate(usdcAddress, wbtcAddress, 10000);
    await mockUniswapIntegration.setExchangeRate(usdcAddress, linkAddress, 10000);
    await mockUniswapIntegration.setExchangeRate(usdcAddress, daiAddress, 10000);
    ...
    // 直接给基金合约铸造代币，避免交换
    await mockWETH.mint(mockFundAddress, ethers.parseUnits("125000", 18)); // 12.5万 WETH
    await mockWBTC.mint(mockFundAddress, ethers.parseUnits("1250", 8)); // 1250 WBTC
    await mockLINK.mint(mockFundAddress, ethers.parseUnits("125000", 18)); // 12.5万 LINK
    await mockDAI.mint(mockFundAddress, ethers.parseUnits("125000", 18)); // 12.5万 DAI
    ...
    // 手动设置基金状态（绕过初始化函数）
    console.log("⚠️  注意：使用简化初始化，跳过代币交换步骤");
    console.log("✅ 基金已准备就绪，可以开始投资");
